...
const UPSTREAM_PATHS = ['schemas/delta.json', 'schemas/discovery.json', 'schemas/hello.json']
...
function toTypebox(schema, context, upstreamPath, indent = 0) {
  ...
  if (schema['$ref']) {
    return resolveRef(upstreamPath, schema['$ref'])
  }
    ...
    case 'object': {
      if (schema.patternProperties && !schema.properties) {
        ...
        if (patterns.length === 1 && patterns[0][0] === '.*') {
          const valExpr = toTypebox(patterns[0][1], `${context}.patternProperties.*`, upstreamPath, indent + 1)
...
async function main() {
  ...
  console.log(`Wrote ${outputs.length} messaging schema modules to ${OUT_DIR}`)
